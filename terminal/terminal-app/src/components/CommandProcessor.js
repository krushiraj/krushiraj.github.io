...
const checkOptKey = (str, ref, context) => {
    ...
    const optLen =
        context["options"] <= Object.getOwnPropertyNames(ref.options).length;
...
const checkArg = (ref, context) => {
    ...
    if (!check) {
        const msg = `Maximum number arguments exceeded. You can pass upto ${
            ref.args
            } arguments only.`;
        context.message = msg;
    }
        ...
        switch (commandTokens[index].type) {
            ...
            case "optkey":
                ...
                obj.options[commandTokens[index].str.slice(2)] =
                    newIndex < commandTokens.length ? commandTokens[newIndex].str : "";
                break;
            case "argument":
...
const handlePermissionDenied = () => {
    ...
    handleError({
        message: `ERROR: ${username}@${sysname}:${pwd}: Permission denied. Cannot ${
            commandTokens[0].str
            }`
    });
};
...
const execute_ls = ({ command }) => {
    ...
    if (dirEnt.type == "file") {
        output = [
            `ERROR: Expected a directory name/path, but found file name/path.`
        ];
    } else {
        output = [".", ".."];
...
const execute_cd = ({ command }) => {
    ...
    if (toDir != undefined) {
        ...
        output = "ERROR: command cd expects a directory name as argument. No such name provided."
    }
    if (output) {
...
const execute_cat = ({ command }) => {
    ...
    if (files[0] == "*") {
        ...
        for (let key in currDir.data) {
            if (currDir.data[key].type == "file") files.push(currDir.data[key].name);
        }
    }
...
const execute_help = ({ command: { args, options } }) => {
    ...
    if (!args.length && !optList.length) {
        output = commands.map(arg =>
            `[${arg.command}] - ${arg.help}`.replace(/\n/g, "\n" + tab)
        );
    } else if (args.length != 1 || optList.length > 1) {
        output = [
            `ERROR: Help command expects exactly one command name as argument and optionally followed by exactly one option in that command.`
        ];
    } else {
...
const execute_color = ({ command: { options } }) => {
    ...
    for (let key in options) {
        ...
        body.style.setProperty(
            `--${key}-color`,
            (_isColor ? "" : "#") + `${options[key]}`
        );
        output.push(
            `Changed ${key} color to ` + (_isColor ? "" : "#") + `${options[key]}`
        );
    }
    paintReadOnly(output.join("\n"));
    paintInputNew();
...
const handleValidCommand = commandTokens => {
    const [command] = commands.filter(
        ({ command }) => command == commandTokens[0]["str"]
    );
...
export class COTrie {
    ...
    buildOptionsTrie() {
        ...
        for (let index in commandList) {
            const options = Object.getOwnPropertyNames(
                commands.filter(arg => arg.command == commandList[index])[0].options
            );
    ...
    getCommandTrie(command) {
        ...
        for (let index in command) {
            if (root.hasOwnProperty(command[index])) root = root[command[index]];
    ...
    getOptionsTrie(command, option) {
        ...
        for (let index in option) {
            if (root.hasOwnProperty(option[index])) root = root[option[index]];
            else return undefined;
        }
        return root;
    }
    ...
    getFilesTrie(filename) {
        ...
        for (let index in filename) {
            if (root.hasOwnProperty(filename[index])) root = root[filename[index]];
            else return undefined;
        }
        return root;
    }
    ...
    getSuggestions(command, option, forCommand) {
        ...
        if (command == "") {
            ...
            this.getStringsFromTrie(
                this.getOptionsTrie(command, option),
                suggestions,
                ""
            );
        }
