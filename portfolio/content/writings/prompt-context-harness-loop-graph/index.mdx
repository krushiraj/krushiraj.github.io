...
description: "Every few months AI grows a new -engineering word. They are not marketing. Each one marks the exact spot where the layer below it leaked, and where control moved one step further out of the model."
...
Graph engineering. Five job titles in about three years, all for talking to the
same box.
...
All five words rest on one fact about the base layer: it predicts the next
...
The five words are five answers to that one question.
...
model to 71.0% on R1-Zero (and 79.8% on R1).
...
[definition](https://x.com/karpathy) is the one everyone quotes, describing
...
Every one of these five words assumes a grader somewhere. Prompt engineering
