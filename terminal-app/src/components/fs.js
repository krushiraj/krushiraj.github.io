...
export default class FSTree {
  ...
  insertNode(treeRoot, node, path) {
    if (!Object.getOwnPropertyNames(treeRoot).length) {
      ...
      if (!treeRoot.data[path[0]]) {
        treeRoot.data[path[0]] = this.insertNode({}, node, path.slice(1));
      } else {
        this.insertNode(treeRoot.data[path[0]], node, path.slice(1));
      }
    }
    return treeRoot;
  }
  ...
  getPathArray(path) {
    const arr = path.split('/').filter(arg => arg !== '');
  ...
  buildFsTree(dirNode, _parent) {
    for (let key in dirNode) {
      const name = key;
      const path = _parent ? _parent.path + '/' + name : name;
      const parent = _parent;
      const type = dirNode[key].type;
      const data = type == 'file' ? dirNode[key].data : {};
      const node = new Node({ name, path, parent, type, data });
      const pathArr = this.getPathArray(path);
      this.fsRoot = this.insertNode(this.fsRoot, node, pathArr);
      if (type != 'file') this.buildFsTree(dirNode[key].data, node);
    }
  }
  ...
  getNodeFromToken(token, pathTokens, index) {
    if (token == '/') {
      ...
      (this._currDir.hasOwnProperty('data') &&
        this._currDir.data.hasOwnProperty(token)) ||
      (token == 'parent' && this._currDir.hasOwnProperty('parent'))
    ) {
      return token == 'parent'
        ? this._currDir.parent
        : this._currDir.data[token];
    } else {
      const at = pathTokens.slice(0, index - 1).join('');
      return {
        type: 'error',
        error: `ENOENT: Error no such file or directory found at ${at}`.replace(
          /parent/g,
          '..'
        )
      };
